...
test_eigengene_explained_variance=function(OriginalData, Original_blockwiseModules, target_module, permutations=999, bicor=FALSE, ...) {
	...
	# Compute module eigengenes for all the modules in the Original_blockwiseModules
	# and select the explained variance for the target module
	} else {
	...
	permuted_indices=lapply(seq_len(permutations), function(X)
							lapply(seq_len(ncol(extract_data_expr_target_module)), function(z)
								sample(seq_len(nrow(extract_data_expr_target_module)),
											nrow(extract_data_expr_target_module), replace=FALSE)
							)	)
	permuted_datasets=lapply(permuted_indices, function(x)
		as.data.frame(matrix(NA, nrow(extract_data_expr_target_module), ncol(extract_data_expr_target_module)))
		)
		# preallocate list
	...
	varExplained_permuted_datasets=unlist(lapply(permuted_datasets, function(X)
				moduleEigengenes(X,colors=colors_permutations,...)$varExplained))
				} else {
	varExplained_permuted_datasets=unlist(lapply(permuted_datasets, function(X)
	...
	Results=list(moduleEigengenes_ALL,
					explained_variance_target_module=observed_explained_variance,
					explained_variance_permuted_datasets=varExplained_permuted_datasets,
